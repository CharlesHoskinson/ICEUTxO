...
%%% ── Title and author ────────────────────────────────────────────────────
\title{ICE-UTxO: Interleaving Coroutine Effects for Proof-Carrying UTxO Transactions}
...
We present ICE-UTxO, a conservative extension of eUTxO in which each transaction is a proof-carrying implementation of a multiparty session protocol. The model adds three layers---coroutine state on UTxOs, transaction-level sequencing via compiled Programmable Transaction Block programs, and Incrementally Verifiable Computation proof artifacts that gate commit---all of which collapse to standard eUTxO when unused. Coordination scripts specify permitted interactions as MPST global types with event-structure semantics; PTB compilation produces deterministic schedules whose conformance is certified by IVC witnesses.

We mechanize the ledger-level operational semantics, conflict serializability, and MPST projection theorems in Lean~4. The development is fully constructive, uses no admitted lemmas or custom axioms, and avoids \texttt{Classical.choice}. Principal results include strong conflict serializability via a constructive bubble-sort proof, concurrent-to-serial refinement via stuttering simulation, and bidirectional MPST-to-ledger bridge theorems establishing that shard-local verification of projected traces is necessary and sufficient for global protocol consistency. Cryptographic primitives and the S-BAC consensus layer are modeled as oracles with explicit security assumptions.

A parallel TLA+ specification (19~modules, 47~safety invariants, 4~liveness properties) validates temporal properties that Lean's logic cannot express: eventual commit under weak fairness, eventual termination of pending transactions, and eventual effect handling. The TLA+ development also models optimistic concurrency with snapshot isolation and provides a formal circuit-to-specification alignment bridge with explicit gap analysis. The two artifacts are complementary: Lean proves safety universally; TLA+ validates liveness under fairness by model checking on bounded instances.
...
\paragraph{Adversary model and trust boundaries.}
The formal guarantees rest on four assumptions, stated here for auditability. \emph{Security Assumption~1 (ZK verifier soundness):} the predicate \texttt{allProofsVerified} is a sound oracle---if it returns true, the attested computation was performed correctly. If this assumption fails, all safety guarantees become vacuous; an adversary can forge proof commitments and commit arbitrary invalid transactions. \emph{Security Assumption~2 (Phase discipline):} the transaction executor enforces the phase-transition ordering (Idle $\to$ Reserve $\to$ Executing $\to$ Committing $\to$ Committed). The Lean mechanization proves safety given this ordering; enforcement is a deployment obligation. \emph{Security Assumption~3 (S-BAC Byzantine tolerance):} each shard has at most $f_s < n_s / 3$ Byzantine validators. Under this bound, S-BAC guarantees safety (no invalid commits); a shard exceeding this bound can force aborts of any transaction touching its state, acting as a censorship vector but not violating safety. \emph{Security Assumption~4 (Collision-resistant hashing):} the frame hash chain provides computational binding under standard cryptographic assumptions.

The Lean mechanization proves properties of the \emph{abstract model}: ledger-level operational semantics, conflict serializability, MPST projection, and invariant preservation. A concrete deployment must additionally implement ZK verification (Assumption~1), enforce phase discipline (Assumption~2), maintain shard validator honesty below the BFT bound (Assumption~3), and enforce lock timeouts via runtime policy (\cref{sec:liveness}).
...
\noindent This matrix is enforced by the \textbf{WF-RoleKind} predicate (\cref{def:well-formedness}).
...
\end{itemize}
Safety properties transfer directly: no-double-spend, consumed monotonicity, and the full ledger invariant hold by \cref{thm:invariant-preservation,thm:no-double-spend,thm:consumed-monotone}, and their restrictions to $\mathit{img}(\iota)$ are exactly the standard eUTxO properties. This argument is a proof sketch, not a mechanized result; the embedding $\iota$ and the step-lifting have not been formalized in Lean. The individual safety theorems it invokes \emph{are} mechanized.
\end{proof}
...
\subsection{TLA+ Specification Architecture}\label{sec:tla-architecture}

The TLA+ specification comprises 19~modules (${\sim}4{,}600$~lines) organized in a layered architecture that parallels the Lean development but addresses a distinct verification concern: behavioral completeness and temporal properties.

\paragraph{Module structure.}
The specification builds from a foundation of type definitions (\texttt{StarstreamTypes}: constants, ranges, enumerations, utility operators) through domain-specific state machines (\texttt{StarstreamFrame}: coroutine execution frames with hash-consistent operations; \texttt{StarstreamUTXO}: the six-state UTxO lifecycle from \texttt{Created} through \texttt{Suspended\_at\_Yield}, \texttt{Reserved}, \texttt{Executing}, \texttt{Suspended\_at\_Effect} to \texttt{Consumed}; \texttt{StarstreamEffects}: per-interface handler stacks and effect routing) to the transaction layer (\texttt{StarstreamCoordination}: five-phase orchestration with PTB integration and IVC configuration; \texttt{StarstreamTransaction}: fourteen-field transaction records with balance checking and proof operations; \texttt{StarstreamAuth}: content-binding signatures with replay protection via chain~ID and block height) to the ledger layer (\texttt{StarstreamLedger}: global state including IEUTxO chunk validity for transaction histories) and finally to the behavioral specifications.

\paragraph{Two behavioral specifications.}
The specification defines two complete state machines. \texttt{StarstreamSpec} models \emph{pessimistic} (lock-based) concurrency: \texttt{ReserveTx} acquires exclusive locks on input UTxOs, and other transactions cannot access locked inputs until the lock holder commits or aborts. \texttt{StarstreamSpecOptimistic} models \emph{optimistic} (snapshot-based) concurrency: transactions execute against local copies of inputs without acquiring locks, and validation occurs at commit time via read-set consistency checks ($\mathit{ReadSetValid}$: every snapshotted UTxO still matches the current ledger state) and output freshness checks ($\mathit{OutputsFresh}$: new output IDs do not collide with existing UTxOs). Both specifications share the same invariant framework and are independently model-checked. The Lean proofs cover only the pessimistic mode; the optimistic mode --- which is the concurrency model that scales in production --- exists only in TLA+.

\paragraph{Refinement.}
\texttt{StarstreamSerial} defines an abstract atomic specification where transactions commit in a single step: the action $\mathit{SerialCommit}(tx)$ atomically consumes inputs, produces outputs, checks balance preservation and authorization, and appends to history. \texttt{StarstreamSerialRefinement} defines the abstraction map from the concurrent to the serial specification: $\mathit{NormalizeState}$ projects intermediate UTxO states (Reserved, Executing, Suspended\_at\_Effect) to \texttt{Suspended\_at\_Yield}; $\mathit{AbsUTXO}$ removes lock annotations. Of the 20+ concurrent actions, 14~are stuttering steps (no abstract state change); only \texttt{CommitTx}, \texttt{FinishRollback}, \texttt{CreateUTXO}, and \texttt{YieldUTXO} are visible at the serial level. The module states $\text{THEOREM}\ \mathit{SerialRefinement} \equiv \mathit{Spec} \Rightarrow \mathit{SerialSpec}$, with the IVC circuit proof serving as the constructive witness for the refinement --- it produces a ZK proof that a specific interleaved execution satisfies the constraints required for commit. This corresponds to Lean's $\mathit{concurrent\_refines\_serial}$ but approaches it from the behavioral (state-machine) side rather than the relational (property) side.

\begin{figure}[t]
\centering
\begin{tikzpicture}[
  mnode/.style={modnode, minimum width=2.6cm, font=\footnotesize},
  >=Stealth,
  node distance=0.8cm
]
%% Foundation layer
\node[mnode, fill=palGrayLt, draw=palGray] (types) {Types};
\node[mnode, fill=palGrayLt, draw=palGray, right=0.6cm of types] (frame) {Frame};
\node[mnode, fill=palGrayLt, draw=palGray, right=0.6cm of frame] (utxo) {UTXO};
\node[mnode, fill=palGrayLt, draw=palGray, right=0.6cm of utxo] (effects) {Effects};
\begin{scope}[on background layer]
  \node[trackgroup, fill=palGray, label={[font=\scriptsize\bfseries]above:Foundation}]
    [fit=(types)(frame)(utxo)(effects)] {};
\end{scope}

%% Protocol layer
\node[mnode, fill=palBlueLt, draw=palBlue, below=1.2cm of types, xshift=0.5cm] (ptb) {PTB};
\node[mnode, fill=palBlueLt, draw=palBlue, right=0.6cm of ptb] (coord) {Coordination};
\node[mnode, fill=palBlueLt, draw=palBlue, right=0.6cm of coord] (proof) {Proof};
\node[mnode, fill=palBlueLt, draw=palBlue, right=0.6cm of proof] (auth) {Auth};
\begin{scope}[on background layer]
  \node[trackgroup, fill=palBlue, label={[font=\scriptsize\bfseries]above:Protocol}]
    [fit=(ptb)(coord)(proof)(auth)] {};
\end{scope}

%% Ledger + Spec layer
\node[mnode, fill=palGreenLt, draw=palGreen, below=1.2cm of ptb, xshift=1.0cm] (tx) {Transaction};
\node[mnode, fill=palGreenLt, draw=palGreen, right=0.6cm of tx] (ledger) {Ledger};
\node[mnode, fill=palGreenLt, draw=palGreen, right=0.6cm of ledger] (spec) {Spec (Pessimistic)};
\begin{scope}[on background layer]
  \node[trackgroup, fill=palGreen, label={[font=\scriptsize\bfseries]above:Ledger \& Behavioral Spec}]
    [fit=(tx)(ledger)(spec)] {};
\end{scope}

%% Verification layer
\node[mnode, fill=palOrangeLt, draw=palOrange, below=1.2cm of tx, xshift=-0.3cm] (session) {SessionProtocol};
\node[mnode, fill=palOrangeLt, draw=palOrange, right=0.4cm of session] (circuit) {CircuitAlignment};
\node[mnode, fill=palOrangeLt, draw=palOrange, right=0.4cm of circuit] (optspec) {Spec (Optimistic)};
\node[mnode, fill=palOrangeLt, draw=palOrange, right=0.4cm of optspec] (serial) {Serial};
\begin{scope}[on background layer]
  \node[trackgroup, fill=palOrange, label={[font=\scriptsize\bfseries]above:Verification}]
    [fit=(session)(circuit)(optspec)(serial)] {};
\end{scope}

%% Top layer
\node[mnode, fill=palPurpleLt, draw=palPurple, below=1.2cm of session, xshift=1.5cm] (refine) {Refinement};
\node[mnode, fill=palPurpleLt, draw=palPurple, right=0.6cm of refine] (inv) {Invariants (47)};
\node[mnode, fill=palPurpleLt, draw=palPurple, right=0.6cm of inv] (mc) {MC (TLC)};
\begin{scope}[on background layer]
  \node[trackgroup, fill=palPurple, label={[font=\scriptsize\bfseries]above:Model Checking}]
    [fit=(refine)(inv)(mc)] {};
\end{scope}

%% Edges
\draw[stdarrow] (types) -- (ptb);
\draw[stdarrow] (frame) -- (coord);
\draw[stdarrow] (utxo) -- (tx);
\draw[stdarrow] (effects) -- (coord);
\draw[stdarrow] (coord) -- (spec);
\draw[stdarrow] (ledger) -- (spec);
\draw[stdarrow] (spec) -- (circuit);
\draw[stdarrow] (spec) -- (inv);
\draw[stdarrow] (spec) -- (optspec);
\draw[stdarrow] (serial) -- (refine);
\draw[stdarrow] (inv) -- (mc);
\end{tikzpicture}
\caption{TLA+ module architecture. The 19~modules are organized in five layers: foundation types, protocol-level state machines, ledger and behavioral specifications, verification modules (session types, circuit alignment, optimistic variant, serial abstraction), and model checking (refinement, invariants, TLC configurations).}
\label{fig:tla-module-deps}
\end{figure}

\subsection{What TLA+ Validates}\label{sec:tla-validates}

\paragraph{Liveness properties.}
The specification defines four liveness properties, each addressing a specific progress risk introduced by ICE-UTxO's concurrency mechanisms. \Cref{tab:liveness} summarizes the properties, the system failures they prevent, the Lean lemmas they depend on, and the fairness assumptions required.

\begin{table}[t]
\caption{TLA+ liveness properties and their relationship to Lean safety lemmas.}
\label{tab:liveness}
\centering\small
\begin{tabular}{lp{3.8cm}lp{2.5cm}}
\toprule
...
L2: Eventual termination & Lock starvation, resource exhaustion & \texttt{abort\_enabled\_of\_pending} & $\mathit{WF}(\text{abort})$ \\
L3: Effect handling & Coroutine deadlock & \texttt{handleEffect\_decreases\_effects} & $\mathit{WF}(\text{handleE})$ \\
L4: Return to idle & Unbounded pending work & (compositional) & $\mathit{WF}(\text{commit}) \wedge \mathit{WF}(\text{abort})$ \\
\bottomrule
\end{tabular}
\end{table}

$\mathit{LIVE\_TxEventuallyCommits}$ states that if the commit action is continuously enabled for a transaction (all preconditions hold persistently), the transaction eventually commits: $\Diamond\Box\,\mathit{CommitEnabled}(\mathit{txId}) \Rightarrow \Diamond\,\mathit{CommittedTxId}(\mathit{txId})$. This is the throughput guarantee --- without it, proof-verified transactions could accumulate in the pending set indefinitely. $\mathit{LIVE\_TxEventuallyTerminates}$ states $\Box(\mathit{PendingTxId}(\mathit{txId}) \Rightarrow \Diamond\,\neg\mathit{PendingTxId}(\mathit{txId}))$: every pending transaction eventually leaves the pending set, either by committing or by aborting. This prevents lock starvation --- without it, a stalled transaction could hold UTxO locks forever, blocking every other transaction that needs those inputs. $\mathit{LIVE\_EffectsEventuallyHandled}$ uses leads-to ($\leadsto$): $\mathit{PendingTxHasEffects}(\mathit{txId}) \leadsto \mathit{PendingTxEffectsCleared}(\mathit{txId})$. This prevents coroutine deadlock --- without it, a suspended coroutine could wait forever for a handler response. The Lean proof $\mathit{handleEffect\_decreases\_effects}$ provides the well-founded descent argument (each handle step strictly shrinks the queue); TLA+ validates that fair scheduling completes the descent. $\mathit{LIVE\_CanReturnToIdle}$ states $\Box\Diamond(\mathit{ledger.pendingTxs} = \{\})$: the system infinitely often reaches a state with no pending transactions, ensuring that work does not accumulate without bound.

\paragraph{Safety invariants.}
The specification defines 47~named safety invariants organized in twelve categories, each protecting a distinct aspect of system correctness. \Cref{tab:tla-invariants} summarizes the categories.
...
The Lean mechanization proves a six-part safety invariant ($\mathit{ledgerInvariant}$); the TLA+ specification checks 47~invariants because it decomposes finer-grained concerns that are implicit in the Lean formalization. For example, the Lean $\mathit{noDoubleSpend}$ predicate corresponds to TLA+'s \texttt{INV\_LINEAR\_NoDoubleSpend}, but TLA+ additionally checks \texttt{INV\_LINEAR\_NoDoubleConsume} (no two committed transactions share an input), \texttt{INV\_LINEAR\_NoDuplication} (no two pending transactions share an input), and \texttt{INV\_LINEAR\_PendingInputsNotConsumed} (pending inputs are not already consumed) --- properties that are consequences of Lean's invariant but whose explicit model checking provides counterexample-search confidence.

\paragraph{Circuit-to-specification alignment.}
The \texttt{StarstreamCircuitAlignment} module is the only formal artifact that connects the ZK interleaving proof circuit to the protocol specification. It provides three mapping tables: circuit operations to TLA+ actions (12~operations including Resume, Yield, Burn, Bind, Unbind, NewUtxo), circuit wires to TLA+ state expressions (14~wire mappings including \texttt{id\_curr}, \texttt{activation}, \texttt{safe\_to\_ledger}, \texttt{value\_commitment}), and circuit constraints to TLA+ invariants (13~constraint-to-invariant mappings). The module also defines eight circuit-specific invariants not present in the original specification: \texttt{INV\_CIRCUIT\_NoSelfResume} (a process cannot resume itself), \texttt{INV\_CIRCUIT\_DualTraceConsistency} (resume inputs match yield outputs --- the core message-matching property of the interleaving proof), and \texttt{INV\_CIRCUIT\_ValueCommitmentIntegrity} (frame hashes bind individual WASM execution proofs to the aggregate interleaving proof), among others.

Critically, the module includes an explicit \emph{gap analysis}: properties that the circuit does \emph{not} enforce and how each gap is resolved. Optimistic concurrency validation, token balance preservation, authorization, liveness, read-set validation, NFT uniqueness, and rollback safety all require ledger-level validation or separate mechanisms. The module defines $\mathit{CircuitVerifiable}(tx)$ (properties the circuit checks), $\mathit{RequiresLedgerValidation}(tx)$ (properties requiring ledger checks), and $\mathit{FullyVerified}(\mathit{ledger}, tx)$ (the conjunction). This makes precise what the Lean proofs treat as an oracle: the TLA+ specification decomposes the ``proof-carrying'' claim into circuit-verified and ledger-verified components.

\paragraph{Session protocol as session types.}
The \texttt{StarstreamSessionProtocol} module formalizes the effect handler protocol as a binary session type. The protocol has three participants: the coroutine (which raises effects and receives resume values), the handler (which receives effects and produces results), and the coordination script (which manages handler installation and dispatch). The module encodes session-type constraints as TLA+ action guards: $\mathit{GuardInstallHandler}$ (install only when no pending effects on the interface), $\mathit{GuardRaiseEffect}$ (raise only when a handler is installed and the source has no pending effect), $\mathit{GuardHandleEffect}$ (handle only when pending effects exist), and $\mathit{GuardProtocolCommit}$ (commit only when all effects are resolved). Linearity constraints ensure each effect is raised, handled, and consumed exactly once. A duality check verifies that for every pending effect, a matching handler exists with the correct effect mask. The module also views the UTxO lifecycle as a session type: $\texttt{Created} \to \texttt{Suspended\_at\_Yield} \to \texttt{Reserved} \to \texttt{Executing} \to \cdots \to \texttt{Consumed}$, with $\mathit{ValidUTXOTransition}$ defining the legal state transitions. This connects the MPST theory in the paper's mathematical sections to the effect system implementation.

\paragraph{Adversarial modeling.}
The behavioral specification includes explicit adversarial actions. $\mathit{InjectInvalidTx}$ models an adversary submitting a malformed transaction (incorrect signatures, violated balance constraints, or missing proof commitments); $\mathit{RejectInvalidTx}$ models the ledger rejecting it. Model checking confirms that no sequence of actions, including adversarial injection, can violate any of the 47~safety invariants.
